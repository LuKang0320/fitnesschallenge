...
services.onPost('/api/fitness/GetFitnessStatistic').reply(async (request) => {
  try {
    await delay(500);
    const { fitid } = JSON.parse(request.data);
    var response = await fetch(process.env.REACT_APP_BASE_URL + '/api/fitness/GetFitnessStatistic?'+ new URLSearchParams({
      fitid: fitid
  }), {
      method:'get'
    });
...
services.onPost('/api/fitness/GetEmployeeProgress').reply(async (request) => {
  try {
    await delay(500);

    const { userid, fid} = JSON.parse(request.data);

    var response = await fetch(process.env.REACT_APP_BASE_URL + '/api/fitness/GetEmployeeProgress?' + new URLSearchParams({
        userid: userid,
        fid: fid
    }), {
      method:'get'
