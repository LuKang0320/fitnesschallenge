...
function ReactTable({ columns, data }) {
  ...
  useEffect(() => {
    setFilter('year', activeTab === 'All' ? '' : activeTab);
    ...
    <>
      ...
      <Stack direction={matchDownSM ? 'column' : 'row'} spacing={1} justifyContent="space-between" alignItems="center" sx={{ p: 3, pb: 3 }}>
        ...
        <Stack direction={matchDownSM ? 'column' : 'row'} alignItems="center" spacing={matchDownSM ? 1 : 0}>
          ...
          <CSVExport data={data} filename={'pastmprs.csv'} />
        </Stack>
      </Stack>
...
const Pastmprs = () => {
  const { GetAllPastMPRs } = useHCSS();
  const [list, setList] = React.useState([]);
  useEffect(() => {
    const init = async () => {  
      let listsres = await GetAllPastMPRs();
